...
namespace Craft
{
		...
		while (true)
		{
			...
			if (deltaTime >= oneFrameTime)
			{
				...
				if (mainLevel)
				{
					...
					// 액터의 이전 상태 저장 처리
					mainLevel->SavePrevActorStates();
				}

				// 입력 상태 저장(Started, OnGoing, Finished, Completed)
	...
	void Engine::PlayerOneShot(const std::string& filename)
	{
		if (!sound)
			return;
	...
	void Engine::PlayerBackgroundMusic(const std::string& filename)
	{
		if (!sound)
			return;
	...
	void Engine::LoadEngineSetting()
	{
		...
		// readSize를 쓰진 않지만 반환 안받아주면 뭐라 해서
